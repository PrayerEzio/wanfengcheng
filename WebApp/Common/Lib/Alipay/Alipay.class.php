...
class Alipay
{
	...
	public function __construct(){
		$this->alipay_config = unserialize(M('Payment')->where(array('payment_code'=>'alipay'))->getField('payment_config'));
		$payment = M('Payment')->where(array('payment_code'=>'alipay'))->getField('payment_config');
		$payment_info['payment_config'] = unserialize($payment);
		/* 支付宝接口配置 */
		$alipay_config['sign_type'] = strtoupper('MD5');
		$alipay_config['input_charset'] = strtolower('utf-8');
		$alipay_config['cacert'] = getcwd().'\\cacert.pem';
		$alipay_config['transport'] = strtolower('http');
		$alipay_config['alipay_partner'] = $payment_info['payment_config']['alipay_partner'];
		$alipay_config['alipay_key'] = $payment_info['payment_config']['alipay_key'];
		$alipay_config['alipay_account'] = $payment_info['payment_config']['alipay_account'];
	...
	public function toAlipay($para)
	{
		...
		$para['partner'] = $this->alipay_config['alipay_partner'];
		$para['payment_type'] = '1';
		$para['seller_email'] = $this->alipay_config['alipay_account'];
		if (empty($para['notify_url'])) {
			$para['notify_url'] = U('Home/Pay/alipayNotify', '', true, true);
		}
		if (empty($para['return_url'])) {
			$para['return_url'] = U('Home/Pay/alipayReturn', '', true, true);
		}
		if (empty($para['_input_charset'])) {
	...
	private function getSignVeryfy($para_temp)
	{
		...
		if ( md5($prestr.$this->alipay_config['alipay_key']) ==  $para_temp['sign'] ) {
			return true;
		} else {
			return false;
		}
	...
	private function createSign($para_sort) {
		//把数组所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串
		$prestr = $this->createLinkstring($para_sort);
		return md5($prestr.$this->alipay_config['alipay_key']);
	}
