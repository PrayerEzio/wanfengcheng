...
class TDESUtil {
	...
	public static function encrypt2HexStr($keys, $sourceData) {
		...
		// 元数据
		$source = $this->ByteUtils->getBytes ( $sourceData );
		
		// 1.原数据byte长度
		$merchantData = count($source);
		// echo "原数据据:" . $sourceData . "\n";
		// echo "原数据byte长度:" . $merchantData . "\n";
		// echo "原数据HEX表示:" . $this->ByteUtils->bytesToHex ( $source ) . "\n";
		// 2.计算补位
		$x = ($merchantData + 4) % 8;
		$y = ($x == 0) ? 0 : (8 - $x);
		// echo ("需要补位 :" . $y . "\n");
		// 3.将有效数据长度byte[]添加到原始byte数组的头部
		$sizeByte = $this->ByteUtils->integerToBytes ( $merchantData );
		$resultByte = array ();
		...
		//echo ("补位后数据HEX表示:" . $this->ByteUtils->bytesToHex ( $resultByte ) . "\n");
		//echo ("秘钥HEX表示:" . $this->ByteUtils->strToHex ( $keys ) . "\n");
		//echo ("秘钥长度:" . count ( $this->ByteUtils->getBytes ( $keys ) ) . "\n");
		//echo $this->ByteUtils->toStr ( $resultByte );
		$desdata = self::encrypt ( $this->ByteUtils->toStr ( $resultByte ), $keys );
		//echo ("加密后的长度:" . strlen ( $desdata ) . "\n");
		return $this->ByteUtils->strToHex ( $desdata );
	}
	...
	public static function decrypt4HexStr($keys, $data) {
		...
		$hexSourceData = $this->ByteUtils->hexStrToBytes ($data);
		// 解密
		$unDesResult = self::decrypt ($this->ByteUtils->toStr($hexSourceData),$keys);
		$unDesResultByte = $this->ByteUtils->getBytes($unDesResult);
		$dataSizeByte = array ();
		for($i = 0; $i < 4; $i ++) {
			$dataSizeByte [$i] = $unDesResultByte [$i];
		}
		// 有效数据长度
		$dsb = $this->ByteUtils->byteArrayToInt( $dataSizeByte, 0 );
		$tempData = array ();
 		for($j = 0; $j < $dsb; $j++) {
 			$tempData [$j] = $unDesResultByte [4 + $j];
 		}
 		
		return $this->ByteUtils->hexTobin ($this->ByteUtils->bytesToHex ( $tempData ));
