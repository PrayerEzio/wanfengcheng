...
class AdvController extends GlobalController {
	...
	public function curd(){
		if (IS_POST) {
			...
			$data['ap_link'] = I('post.ap_link','','url');
			$data['ap_intro'] = str_rp(trim($_POST['ap_intro']));
			$data['ap_sort'] = I('post.ap_sort',0,'int');
			$data['is_use'] = I('post.is_use',0,'int');
 			if ($id) {
				$this->model->where(array('ap_id'=>$id))->save($data);
			}else {
				$id = $this->model->add($data);
			}
			if(!empty($_FILES['default_pic']['name']))
			{
				$param = array('savePath'=>'info/','subName'=>'','files'=>$_FILES['default_pic'],'saveName'=>'info_'.$id,'saveExt'=>'');
				...
				if($up_return == 'error')
				{
					...
					$this->model->where(array('ap_id'=>$id))->setField('ap_pic',$up_return);
				}
			}
			$this->success('操作成功',U('adv_position'));
		}elseif (IS_GET){
			$id = I('get.id',0,'int');
			if ($id) {
				$info = $this->model->where(array('ap_id'=>$id))->find();
