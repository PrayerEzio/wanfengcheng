...
class GlobalController extends Controller {
    ...
    protected function _initialize()
	{
		...
		if ($admin_type) {
			//维修商
			if (CONTROLLER_NAME != 'Feedback' || ACTION_NAME == 'allot') {
    ...
    public function getDistrict(){
    	if (IS_AJAX) {
    		$id = intval($_POST['id']);
    		$list = M('District')->where(array('upid'=>$id))->order('d_sort')->select();
    		if (is_array($list)) {
    			if ($list[0]['level'] == 2) {
    				$info = '<select name="city_id" id="city" onchange="getDistrict(\'city\')">';
    			}else {
    				$info = '<select name="area_id" id="area">';
    			}
