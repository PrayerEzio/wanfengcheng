...
class SettingController extends GlobalController {
	...
	public function base_information()
	{
		...
		if(IS_POST)
		{
			...
			$data['statistics_code'] = str_replace('\'','&#39;',$_POST['statistics_code']);//htmlspecialchars(trim($_POST['statistics_code']));
	...
	public function admin_list()
	{
		...
		$admin_type = I('get.type',0,'int');
		$this->search = $_GET;
		switch($op)
		{
			case 'list':
				$list = $Admin->where(array('admin_type'=>$admin_type))->select();
			...
			case 'edit':
				if(IS_POST)
				{
					...
					if(trim($_POST['admin_pwd']))
					{
						...
						$Admin->where('admin_id='.$admin_id)->setField('admin_type',$admin_type);
					}
					//权限
					...
					$admin_auths = M('AdminAuth')->where('a_default=0 and a_show=1')->order('a_sort asc')->select();
					$admin_auth = array();
					foreach ($admin_auths as $key => $val){
						$admin_auth[$val['auth_class']][] = $val;
					}
					$this->admin_auth = $admin_auth;
					$admin_id = intval($_GET['admin_id']);
					$vo = $Admin->where('admin_id='.$admin_id)->find();
					$this->auth = explode(',',$vo['admin_auth']);
					//p($this->auth);p($vo);die;
					$this->assign('vo',$vo);	
		...
		switch($op)
		{
			...
			case 'edit':
				if(IS_POST)
				{
					...
					$a_data['auth_class'] = str_rp($_POST['auth_class'],1);
			...
			case 'add':	
				if(IS_POST)
				{
					...
					$data['auth_class'] = str_rp($_POST['auth_class'],1);
					$AdminAuth->add($data);
