		...
		if(IS_POST)
		{
			...
			if($doc_id)
			{
				...
				if(!empty($_FILES['doc_pic']['name'])){
					$arc_img = $Document->where(array('doc_id'=>$doc_id))->getField('doc_code');
					$param = array('savePath'=>'artic/','subName'=>'','files'=>$_FILES['doc_pic'],'saveName'=>$arc_img,'saveExt'=>'');
					$up_return = upload_one($param);
					if($up_return == 'error')
					{
						$this->error('图片上传失败');
